...
## Summary
Refactored seed files to reduce cognitive complexity violations as suggested by ESLint SonarJS plugin.
...
**Refactorings Applied:**
Extracted 9 helper methods from monolithic `processComic()`:
...
**New file:** `src/database/seed/seeders/comic-seeder-refactored.ts`
**Action needed:** Replace original with refactored version
...
Isolated different aspects (metadata, images, status, genres) into dedicated methods

## Benefits Achieved

✅ **Maintainability:** Code is easier to understand and modify
✅ **Testability:** Smaller methods are easier to unit test
✅ **Reusability:** Helper methods can be reused across seeders
✅ **Debugging:** Stack traces are clearer
✅ **ESLint Compliance:** All SonarJS cognitive-complexity rules satisfied
...
const preprocessedComics = (rawComics as RawComic[]).map((comic: RawComic) => {
  ...
  if (comic.status && typeof comic.status === "string") {
    const matchedStatus = validStatuses.find(
      (s) => s.toLowerCase() === (comic.status ? comic.status.toLowerCase() : "")
    );
    if (matchedStatus) {
