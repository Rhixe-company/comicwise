...
| 6   | Script Validation          | ✅ Complete | 20 min   | package.json, scripts/_.ps1, scripts/_.sh        |
...
**Validation:** \`\`\`bash pnpm lint --quiet # Should pass \`\`\`
...
**Usage:** \`\`\`bash pnpm tsx scripts/consolidate-types.ts --verbose \`\`\`
...
**Suggested Replacements:** | Context | Old | New | |---------|-----|-----| |
Request | `any` | `Request \\| NextRequest` | | Response | `any` |
`Response \\| NextResponse` | | Error | `any` | `Error \\| unknown` | | Data |
`any` | `unknown` | | Params | `any` | `Record<string, string \\| string[]>` |
...
- [x] Legacy support (src/_, components/_, etc.)
...
**ESLint Rule:** \`\`\`typescript "unicorn/filename-case": [ "warn", { cases: {
kebabCase: true, pascalCase: true, camelCase: true, }, ignore: [
"^[A-Z]._\\\\.tsx?$", // Components "^use[A-Z]._\\\\.tsx?$", // Hooks ], }, ],
\`\`\`
...
**Usage:** \`\`\`bash pnpm scaffold <type> <name> \`\`\`
...
See [FINAL_DELIVERY_SUMMARY.md](FINAL_DELIVERY_SUMMARY.md) for detailed next
steps.
