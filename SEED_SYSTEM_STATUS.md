...
The existing `run-optimized.ts` is production-ready and has been battle-tested. Use it for all seeding operations.

The `run-ultra-optimized.ts` was created as an experimental alternative with more flexible schemas, but the existing system is sufficient for all current needs.

**Recommendation: Use `pnpm db:seed` which uses the tested and working seed system.**
