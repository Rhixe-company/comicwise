...
## Overview
Optimized the `@/src/database/seed/` files to use the `@/database/queries` and `@/database/mutations` layers instead of direct database queries.
...
- Added `getChapterByComicAndNumber()` - Find chapters by comic ID and chapter number
...
- **Type Safety**: Better TypeScript integration with explicit function signatures

## Backward Compatibility

All changes are backward compatible. The seeders continue to work the same way, just with cleaner internal implementation.
