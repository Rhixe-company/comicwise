...
import type { Comic } from '#types/database-auto';
...
export class ComicDal {
  ...
  async findById(id: string): Promise<Comic | undefined> {
    try {
  ...
  async update(id: string, data: Partial<typeof comic.$inferInsert>): Promise<Comic | undefined> {
    try {
  ...
  async delete(id: string): Promise<Comic | undefined> {
    try {
  ...
  async list(options: {
    ...
    try {
      ...
      if (type) {
        conditions.push(eq(comic.type, type));
      }
  ...
  async assignGenres(comicId: string, genreIds: string[]): Promise<void> {
    try {
      ...
      if (genreIds.length > 0) {
        await db.insert(comicToGenre).values(
          genreIds.map((genreId) => ({ comicId, genreId }))
        );
      }
