...
class DocumentationGenerator {
  ...
  private addSection(title: string, content: string, level: number = 1) {
...
\`\`\`
  ...
  private async saveDocumentation(): Promise<void> {
    let markdown = "# ComicWise - Complete Documentation\n\n";
    ...
    const docPath = path.join(docsDir, "COMPREHENSIVE_GUIDE.md");
    fs.writeFileSync(docPath, markdown);

    this.log(`✓ Documentation saved: ${path.relative(ROOT_DIR, docPath)}`, "success");
  }

  async run(): Promise<void> {
    console.log(chalk.cyan("═".repeat(60)));
    console.log(chalk.cyan("  ComicWise - Documentation Generator"));
    console.log(chalk.cyan("═".repeat(60)) + "\n");
