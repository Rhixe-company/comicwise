...
async function analyzeTypeScript(): Promise<void> {
  const spinner = ora("Analyzing TypeScript (strict)...").start();
...
async function analyzeESLint(): Promise<void> {
  const spinner = ora("Analyzing ESLint (strict)...").start();

  try {
    const { stdout } = await execAsync("pnpm lint:strict", { cwd: ROOT_DIR });
...
function generateReport(): string {
  return `# ComicWise Project Analysis Report
