  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--collection" || a === "-c") {
      ...
      switch (a) {
        ...
        default:
          if (!a.startsWith("-") && !out.collection) {
            out.collection = a;
          }
      }
  }
...
function convertCollectionToPlugin(collectionId, mode = "migrate", silent = false) {
  ...
  console.log("\n📊 Summary:");
  if (stats.prompts > 0) console.log(`   - Commands (prompts): ${stats.prompts}`);
  ...
  if (stats.prompts > 0) console.log(`   - ${path.join(pluginDir, "commands", "*.md")}`);
  if (stats.agents > 0) console.log(`   - ${path.join(pluginDir, "agents", "*.md")}`);
  if (stats.skills > 0) console.log(`   - ${path.join(pluginDir, "skills", "*")}`);
...
async function main() {
  try {
    ...
    if (!collectionId) {
      ...
      const num = Number.parseInt(collectionId, 10);
