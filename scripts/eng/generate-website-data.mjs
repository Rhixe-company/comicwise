...
function generateAgentsData(gitDates) {
  ...
  for (const file of files) {
    ...
    const relativePath = path.relative(ROOT_FOLDER, filePath).replaceAll("\\", "/");
...
function generateSkillsData(gitDates) {
  ...
  for (const folder of folders) {
    ...
    if (metadata) {
      const relativePath = path.relative(ROOT_FOLDER, skillPath).replaceAll("\\", "/");
